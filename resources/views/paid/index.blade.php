...
@section('main')
    <div class="page-heading">
        ...
        <section class="section">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item"
                                            href="{{ route('report', ['par' => 'payment', 'pile' => 'xls']) }}">Excel</a>
                                        <a class="dropdown-item"
                                            href="{{ route('report', ['par' => 'payment', 'pile' => 'pdf']) }}"
                                            href="#">PDF</a>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    @foreach ($payment as $item)
                                        @if ($item->grant == null)
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link {{ $loop->first ? 'active' : null }}" id="home-tab"
                                                    data-bs-toggle="tab" href="#tab{{ $item->id }}" role="tab"
                                                    aria-controls="home" aria-selected="true">{{ ucfirst($item->name) }}</a>
                                            </li>
                                        @endif
                                    @endforeach
                                    <a class="nav-link" id="home-tab" data-bs-toggle="tab" href="#tablpk" role="tab"
                                        aria-controls="home" aria-selected="true">LPK</a>
                                </ul>
                                <div class="tab-content" id="myTabContent">
                                    @foreach ($payment as $var)                                
                                        @if ($var->grant == null)
                                            <div class="tab-pane fade {{ $loop->first ? 'show active' : null }}"
                                                id="tab{{ $var->id }}" role="tabpanel" aria-labelledby="home-tab">
                                                <br>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <a data-bs-toggle="modal"
                                                                                href="#open{{ $item->id }}">
                                                                                <img src="{{ asset('assets/image/' . $item->img) }}"
                                                                                    class="imgs">
                                                                            </a>
                                                                        </td>
                                                                        <td>{{ number_format($item->payment->nom, 0, ',', '.') }}
                                                                        </td>
                                                                        <td>
                                                                            @if ($item->users->siswa)
                                                                        ...
                                                                        <td>
                                                                            @if ($item->status == 0)
                                                                                <span class="badge bg-warning">On
                                                                                    progress</span>
                                                                        ...
                                                                        @if (auth()->user()->role == 'keuangan')
                                                                            <td>
                                                                                ...
                                                                                @else
                                                                                    <button type="button"
                                                                                        class="btn btn-sm btn-success rounded-pill">Telah
                                                                                        diverifikasi</button>
                                                                                @endif
                                                                            </td>
                                                                        @endif
                                    ...
                                    <div class="tab-pane fade" id="tablpk" role="tabpanel" aria-labelledby="home-tab">
                                        <br>
                                        <div class="table-responsive">
                                            <table class="table" id="table1">
                                                <thead>
            ...
            @foreach ($da as $item)
                <div class="modal fade" id="open{{ $item->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
                                ...
                                <div class="container">
                                    ...
                                    <div class="d-flex justify-content-start">
                                        ...
                                        <button type="button" class="btn btn-danger rounded-pill" data-bs-toggle="modal"
                                            href="#reject{{ $item->id }}">Tolak</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal fade" id="reject{{ $item->id }}" data-bs-backdrop="static"
